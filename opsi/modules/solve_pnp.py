...
def post_hook():
    class SolvePNP(Function):
        @dataclass
        class Settings:
            calibration_file: get_calibration_files(persist=HookInstance.persist)
        ...
        def on_start(self):
            calib_file_name = str(
                HookInstance.persist.get_calibration_file_path(
                    self.settings.calibration_file
                )
            )
    ...
    class Position2D(Function):
        ...
        class VisualizeTargetPose(Function):
            ...
            def on_start(self):
                ...
                ) = read_calibration_file(calib_file_name)
